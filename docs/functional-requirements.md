...
| 版本 | 日期 | 描述 | 作者 |
|------|------|------|------|
| 1.48 | 2026-08-18 | **质量提升续轮（冻结功能清单，仅重构/清理/修复，无功能变更）**：① **读回校验补 0**（F-BAT-01 契约）：`apply_battery_state` 读回此前只拒 >100，损坏的 0 会被当作"合法 0%"持久化（care=true+limit=0）——补齐拒绝并加回归测试；"满充=100"收敛为 `limits::FULL_CHARGE_LIMIT` 单点（原散落 5+ 处字面量）。② **WMI 延迟恢复探测预算修复**：`pending_backend_switch` 判定移到探测预算递增**之前**——手动切换在飞时被跳过的探测不再消耗预算，末次探测不再被静默取消（F-HAL-18 契约对齐）。③ **Fn 绑定类名规范化**：`valid_class` 按 trim 校验但存储未 trim、带首尾空白的类名"校验通过却永不命中订阅类"——消毒与 GUI 添加先 trim 再存。④ **battery_health 通道关闭真正终止轮询**：`send_or_finish` 返回 bool，GUI 通道消失后线程退出（原文档声称"优雅结束"但循环从未退出）。⑤ **死代码/命名清理**：移除从未被读取的 `egui_ctx` 字段与 `RuntimeState` 无用 `PartialEq`；`consecutive_hw_failures` 更名 `consecutive_read_failures`（仅计读取，与文案一致）。⑥ **错误文案单一来源 + 中文化**：commands.rs 重复的用户可见文案收敛为单一来源（format! 用函数 + 无参用常量）；winring0/wmi 用户可见英文错误改中文（NFR-UX-03）。⑦ **魔法数收敛**：VARIANT 类型号改 `VT_*` 常量、电源未知哨兵 `255` 命名、托盘居中回退 320/200 命名、battery_health 快/慢退避命名。⑧ **托盘可诊断性**：NIM_ADD 失败携带 `GetLastError`、SetTimer 失败告警、`toggle_main_window` 只查一次可见性、`power_broadcast_to_command` 注释修正门控位置。⑨ **窗口/消息窗**：`GetMessageW` 错误路径补 `DestroyWindow`（防 HWND 泄漏）、hide/show 的 EXSTYLE 读-改-写收敛为 `set_exstyle_bits` 且 `GetLastError` 只取一次、`unsafe` 块收缩到 FFI 调用。⑩ **mock 去重**：三个 getter 的读开关与 0/>100 判定收敛为 `ensure_readable`/`validate_read_raw`；`quantizing`/`charge_limit_fails` 去掉冗余初始化。⑪ **测试基线**：破坏性计划任务往返测试补 `#[ignore]`（仅 env 门控不足以防 `cargo test` 误删真实任务）、power 模块依赖共享闩的两个测试加互斥（消除并发 flake）。**验证**：277 项全绿 + clippy/fmt 清零 | opencode |
