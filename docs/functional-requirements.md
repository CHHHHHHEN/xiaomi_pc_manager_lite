...
| 版本 | 日期 | 描述 | 作者 |
|------|------|------|------|
| 1.51 | 2026-08-19 | **收敛与修复轮（冻结功能清单，仅重构/清理/修复）**：① **最小化吞托盘通知**（F-TRAY-13/14）：`refresh_tray_tooltip` 的"窗口可见不弹"门控用 `main_window_visible`——最小化到任务栏的窗口按可见计（`window_at_hidden_pos` 排除 IsIconic），最小化期间性能/养护/充电上限气泡全部静默丢失（用户看不到 GUI 反而收不到通知）。新增 `platform::window::window_visible_to_user`（在屏、非最小化、非离屏隐藏才视为"用户可见"），托盘门控改用之；托盘 toggle 仍用 `main_window_visible`（最小化窗口点击托盘 → 隐藏到托盘，语义不变）。② **退出丢在飞自启动操作**（F-AUTO-03）：取消勾选自启动后立刻退出，DeleteTask 尚未执行完进程即结束——任务残留而配置已落盘为关，下次启动 sync 对"配置关 + 任务在"保守不删，任务永久残留。`handle_quit` 先有界等待 `autostart_in_flight` 归零（3s）并处理 `SetAutostartResult` 回执再退出。③ **充放电速率负值毁掉整条 ETA**（F-BAT-15 健壮性）：`BatteryStatus.ChargeRate/DischargeRate` 固件以有符号 Int32 承载，"该方向未充放电"可能上报负值——原 `uint_prop` 对 VT_I4 负值判"不可读"，整条 `BatteryStatus` 变 None、充放电两条 ETA 一起丢失；新增 `win::variant::unsigned_rate_from_variant`（VT_I4 负值钳为 0 = 无速率，`eta_*` 自然不展示）与 `uint_rate_prop`。④ **捕获"使用此键"绑定释放事件**（F-FNK-06）：捕获开启时按键已按住、收到的首条事件是释放（`012800`）——绑定仅命中未来释放、下次物理按键（`012801`）永不触发；`capture_prefix` 把释放前缀归一化为按下（状态字节 `00`→`01`）。⑤ **收敛**：`util::write_utf16_capped`（托盘 szInfoTitle/szInfo/szTip 三处手写"encode_utf16.take+NUL"统一，NUL 是 Windows 宽字符串唯一终止信号）；`win::com::select_all_wql`（fn_watcher 事件订阅与 battery_health 快照查询两处 `SELECT * FROM {}` 统一）；`app::fnkey::binding_label`（绑定列表/捕获行/`FnKeyBinding::label` 三处展示格式统一，绑定列表不再为取 label 临时构造整条 `FnKeyBinding`）；`ec::embed::atomic_write` 委托包装删除（winring0/embed 直接 `util::fs::atomic_write`，模块文档同步）；`KNOWN_FN_KEYS` 的 Fn+K 条目改引用 `FN_K_WMI_CLASS`/`FN_K_PRESS_PREFIX` 常量（消除字面量漂移，新增锁定测试）；`battery.rs` 读回契约注释重复收敛。⑥ **文档-代码契约对齐**：Cargo.toml NFR-PERF-04 目标注释（≤5MB → Could ≤6MB）、`app/limits.rs` 模块文档（`ec::config`/`ec::battery` → `app::config`/`app::battery`）、`gui/commands.rs` 文档（`ec::fnkey` → `app::fnkey`）、README 模块树补 `util/fs.rs`、SRS NFR-MNT-02 寄存器地址位置（`ec/mod.rs` → `ec/addr.rs`）、SRS 修订记录 1.30 重复句删除。**验证**：本机 288 项单测（新增 4 项：速率钳制、捕获前缀归一化、已知键与常量一致、UTF-16 定长写入）+ clippy/fmt 清零，无功能行为变更 | opencode |
...
| NFR-MNT-02 | EC 寄存器地址应在 `ec/addr.rs` 中以命名常量集中管理 | Should |
