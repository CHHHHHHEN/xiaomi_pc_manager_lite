//! 组合根启动编排：日志、panic 兜底、单实例、提权、后端初始化与 GUI 启动。
//!
//! 历史实现把这些逻辑全部放在 `main.rs`（二进制入口），库目标无法复用。
//! 收敛到本模块后 `main.rs` 退化为薄壳（`run()`），启动流程可被单元测试与
//! 其它工具复用。
...
fn degraded_result(
    ...
    app::startup::StartupResult {
        backend: Box::new(crate::ec::backend::NullBackend),
        config: config.clone(),
...
/// 应用入口（`main.rs` 的薄壳只调用本函数）。
///
/// 流程：单实例预检 → 提权 → 单实例保护 → 配置加载 → 后端初始化（独立线程，
/// 阻塞等待）→ 自启动同步 → GUI 启动。
pub fn run() {
    init_logging();
    init_panic_hook();
    ...
    // 互斥体立即释放、单实例保护失效），用 let 绑定到函数作用域末尾。
    ...
    // 该线程的 COM 由 autostart::sync 的 ComScope 自行初始化/配对回收）。
    {
        ...
        if let Err(e) = crate::util::spawn_guarded("autostart-sync", move || {
            if let Err(e) = crate::platform::autostart::sync(cfg.auto_start_on_boot) {
    ...
    // 传播到主线程——release 构建无控制台，进程无声退出，连精心设计的
    // NullBackend 兜底都到不了。改为在线程内 `catch_unwind`：panic 被捕获
    // 后按"后端不可用"优雅降级，GUI 照常启动并展示错误。
